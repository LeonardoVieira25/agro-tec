...
function ResumoAnalise() {
  const temperaturaMedia = (
    data.reduce((sum, item) => sum + item.temperatura, 0) / data.length
  ).toFixed(2);
  const precipitacaoTotal = data.reduce(
    (sum, item) => sum + item.precipitacao,
    0
  );
  const deficitHidricoTotal = data.reduce(
    (sum, item) => sum + item.deficitHidrico,
    0
  );
  const custoTotal = numberFormatter(
    data.reduce((sum, item) => sum + item.custo, 0)
  );
  ...
  return (
    ...
    >
      ...
      <Typography>
        ...
        {data.reduce((sum, item) => sum + item.temperatura, 0) / data.length}°C,
        enquanto a precipitação total acumulada no período foi de{' '}
        {data.reduce((sum, item) => sum + item.precipitacao, 0)} mm. Esses
        fatores influenciam diretamente a evapotranspiração, que apresentou
        valores médios de{' '}
        {data.reduce((sum, item) => sum + item.evapotranspiracao, 0) /
          data.length}{' '}
      ...
      <Typography>
        ...
        {numberFormatter(data.reduce((sum, item) => sum + item.custo, 0))},
